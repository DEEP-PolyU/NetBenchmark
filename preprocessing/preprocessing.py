...
def mask_fivefold_val_edges(adj):
    ...
    test_edge_idx = all_edge_idx[num_val+randomint:(num_val+randomint + num_test)]
    test_edges = edges[test_edge_idx]
    val_edges = edges[val_edge_idx]
    train_edges = np.delete(edges, test_edge_idx, axis=0)

    def val_true_false(adj, val_edges):
        ...
        while len(val_edges_false) < len(val_edges):
            ...
            return val_edges_false
    val_edges_false=val_true_false(adj,val_edges)

    # NOTE: these edge lists only contain single direction of edge!
    return val_edges, val_edges_false,



def mask_val_test_edges(adj):
