...
def load_citation_2graph(dataset_str="cora", normalization="AugNormAdj", use_feat=True, cuda=True):
    ...
    if dataset_str == 'citeseer':
        ...
        adj, features, adj2 = preprocess_citation_graph(adj, features, normalization)
    ...
    return adj, features, labels, idx_train, idx_val, idx_test, adj2


def load_citation_bigraph(dataset_str="cora", normalization="AugNormAdj", use_feat=True, cuda=True):
    """
    Load Citation Networks Datasets.
    """
    names = ['x', 'y', 'tx', 'ty', 'allx', 'ally', 'graph']
