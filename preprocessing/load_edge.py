...
def load_edgedata(dataset_name):
        ...
        features = np.array(
            [features for _, features in sorted(G.nodes(data='features'), key=lambda x: x[0])],dtype=np.float64)
        labels = np.array(
        ...
        adj = adj.tocsc()
        features = sp.csc_matrix(features)  # change to csc
