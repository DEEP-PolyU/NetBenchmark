...
class HomeGraph(object):
    def __init__(self, srcs, dests, value=None, features=None):
        """
        srcs: np.array of shape [n,], dtype np.int/np.int32
        dsts: np.array of shape [n,], dtype np.int/np.int32
        feature: list of length n with each element is np.array of shape [d, ] np.flot
        """
        self.src = srcs
        self.dest = dests
        self.graph = {}
        ...
        self.node_len = len(self.graph)
        self.feature = features
        self.src_unique = list(self.graph.keys())

    def construct_graph(self):
        for i, src_ in enumerate(self.src):
            dest_ = self.dest[i]
            if src_ not in self.graph:
                self.graph.setdefault(src_, [])
            self.graph[src_].append(dest_)
            if dest_ not in self.graph:
                self.graph.setdefault(dest_, [])
        ...
        for key, value in self.graph.items():
            ...
            self.graph[key] = value_set
            num_neigh += len(value_set)
        self.avg_neigh = (1.0 * num_neigh) / len(self.graph)
    ...
    def __getitem__(self, idx):
        ...
        if self.feature is not None:
            for neigh_ in self.graph[idx]:
                feat.append(self.feature[neigh_])
            feat = np.array(feat)
        return self.graph[idx], (None if self.feature is None else feat)

    def __iter__(self):
        for i in range(self.node_len):
            yield self[i]

    def get_neighbor_by_src(self, src, feature=False):
        neighbors = self.graph[src]
        if len(neighbors) == 0:
            neighbors.append(src)
            print('!!! isolated node %s' % src)
        feat = []
        if feature:
            for neigh_ in neighbors:
                feat.append(self.feature[neigh_])
            feat = np.array(feat)
            return neighbors, feat
        else:
            return neighbors, feat
...
class BiGraph(object):
    def __init__(self, srcs, dests, edge_data=None, features=None):
        """
        srcs: np.array of shape [n,], dtype np.int/np.int32
        dsts: np.array of shape [n,], dtype np.int/np.int32
        feature: list of length n with each element is np.array of shape [d, ] np.flot
    ...
    def construct_graph(self):
        for i, src_ in enumerate(self.src):
            ...
            if src_ not in self.graph:
                self.graph.setdefault(src_, [])
            self.graph[src_].append((dest_, self.data[i]))
...
class EdgeTable_BP(object):
    ...
    def load(path):
        with open(path, 'rb') as f:
            data = pickle.load(f)
        ret = EdgeTable_BP(data['src'], data['dest'], data['data'], data['n_nodes'])
    ...
    def reverse(self):
        # print(self.n_nodes)
        tbl = EdgeTable_BP(self.dest, self.src, edge_data=self.data, n_nodes=self.n_nodes)
