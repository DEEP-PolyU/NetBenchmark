...
from .sampler import IterateNodeSampler, RandomNHopNeighborSampler, RandomNegativeSampler, NodeSampler, \
    RandomNHopNeighborSampler_homoge, RandomNHopNeighborSampler_BI
from .utils import load_citation_agcr
...
class GraphInput(object):
    ...
    def _feed_next_sample(self, node_iter):
        src_id, src_y, hop_neigh = self._next_sample_public(node_iter)
        feat_list = self.gather_feat([src_id] + hop_neigh)
        src_y = torch.LongTensor(src_y)
        return feat_list, src_y
...
class GraphInput_GCR(object):
    ...
    def __init__(
        ...
        self.read_data_from_edges()
        self.u_neighs_num = [self.avg_neigh for i in range(self.u_depth)]
